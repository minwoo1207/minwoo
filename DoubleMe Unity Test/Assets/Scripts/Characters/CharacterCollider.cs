    ...
    protected void OnTriggerEnter(Collider c)
    {
        if (c.gameObject.layer == k_CoinsLayerIndex)
		{
            ...
            else
            {
                ...
                PlayerData.instance.coins += 1 * TrackManager.instance.multiplier;
				controller.coins += 1 * TrackManager.instance.multiplier;
        ...
        else if(c.gameObject.layer == k_ObstacleLayerIndex)
        {
            ...
            {
				if (controller.shieldLife)
                {
					controller.shieldLife = false;
				}
				else
                {
